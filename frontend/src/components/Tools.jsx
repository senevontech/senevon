...
import { FaJava, FaChartLine, FaChartBar, FaDatabase, FaBrain } from "react-icons/fa";

/* ---------- split text into spans for GSAP ---------- */
function splitToSpans(text, cls) {
  // Fast + predictable; avoids Array.from overhead on large strings (still safe for titles)
  const out = [];
  for (let i = 0; i < text.length; i++) {
    const ch = text[i];
    if (ch === " ") out.push(<span key={i}>&nbsp;</span>);
    else out.push(<span key={i} className={cls}>{ch}</span>);
  }
  return out;
}

/* ---------- safe icon render (prevents crashes) ---------- */
const SafeIcon = memo(function SafeIcon({ Icon }) {
  if (!Icon) return <span className="ts-fallbackIcon">◆</span>;
  return <Icon />;
});

/* ---------- one chip ---------- */
const Chip = memo(function Chip({ icon: Icon, label }) {
  return (
    <div className="ts-chip">
      <span className="ts-chipIcon" aria-hidden="true">
        <SafeIcon Icon={Icon} />
...
/* ---------- marquee row (seamless loop) ---------- */
const MarqueeRow = memo(function MarqueeRow({ title, subtitle, items, dir = "rtl", speed = 26 }) {
  // Duplicate once for seamless 50% translate loop (CSS)
  const doubled = useMemo(() => {
    // Avoid reallocation when items stable
  ...
  return (
    <div className="ts-row">
      ...
      <div className="ts-marquee" data-dir={dir} style={{ "--ts-speed": `${speed}s` }}>
        <div className="ts-track">
          {doubled.map((it, idx) => (
            <Chip key={`${it.label}-${idx}`} icon={it.icon} label={it.label} />
          ))}
...
export default function ToolsStackSection({
  ...
  const titleSpans = useMemo(() => splitToSpans(title, "ts-letter"), [title]);
  const subtitleSpans = useMemo(() => splitToSpans(subtitle, "ts-subLetter"), [subtitle]);

  // Keep rows stable forever (icons are module-level, safe)
  const rows = useMemo(
    () => [
      {
        title: "LANGUAGES",
  ...
  // GSAP reveal on enter (optimized)
  useEffect(() => {
    const root = sectionRef.current;
    if (!root) return;

    // If user prefers reduced motion, skip any JS animation work.
    ...
    const io = new IntersectionObserver(
      (entries) => {
        const ent = entries[0];
        if (!ent?.isIntersecting || killed) return;

        // Scope animations to this component only (safe cleanup)
        ctx = gsap.context(() => {
          const tLetters = titleRef.current?.querySelectorAll(".ts-letter");
          const sLetters = subRef.current?.querySelectorAll(".ts-subLetter");
          const blocks = root.querySelectorAll(".ts-row");

          // Avoid doing work if refs not ready
          if (!tLetters?.length && !sLetters?.length && !blocks?.length) return;

          gsap.set([tLetters, sLetters], { opacity: 0, y: 18, rotateX: 55, transformPerspective: 600 });
          gsap.set(blocks, { opacity: 0, y: 16 });

          gsap
            .timeline({ defaults: { ease: "power3.out" } })
            .to(tLetters, { opacity: 1, y: 0, rotateX: 0, duration: 0.7, stagger: 0.03 })
            .to(sLetters, { opacity: 1, y: 0, rotateX: 0, duration: 0.45, stagger: 0.012 }, "-=0.28")
            .to(blocks, { opacity: 1, y: 0, duration: 0.45, stagger: 0.08 }, "-=0.12");
        }, root);
